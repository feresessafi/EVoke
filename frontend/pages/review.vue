      ...
      <div class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card>
            <CardHeader class="space-y-1">
              ...
              <CardDescription>
                <span class="text-2xl font-bold">{{ reviewStats.pending }}</span>
          ...
          <Card>
            <CardHeader class="space-y-1">
              ...
              <CardDescription>
                <span class="text-2xl font-bold text-green-600">{{ reviewStats.approved }}</span>
                <span class="text-xs text-muted-foreground ml-2">94% accuracy</span>
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader class="space-y-1">
              ...
              <CardDescription>
                <span class="text-2xl font-bold text-yellow-600">{{ reviewStats.needsRevision }}</span>
                <span class="text-xs text-muted-foreground ml-2">-2 from yesterday</span>
              </CardDescription>
            </CardHeader>
          </Card>
          <Card>
            <CardHeader class="space-y-1">
              <CardTitle class="text-sm font-medium">Avg. Review Time</CardTitle>
              <CardDescription>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Review and manage generated articles</CardDescription>
          </CardHeader>
          <CardContent>
            <div class="space-y-4">
              <div v-for="article in filteredArticles" 
                   :key="article.story_title" 
                   class="group flex items-start gap-4 p-4 bg-zinc-100 dark:bg-zinc-900 rounded-lg 
                ...
                <div class="mt-1">
                  <div :class="{
                    ...
                    'bg-yellow-500 group-hover:bg-yellow-600': article.status === 'review_needed',
                    'bg-green-500 group-hover:bg-green-600': article.status === 'published',
                    'bg-red-500 group-hover:bg-red-600': article.status === 'draft'
                  }"></div>
                </div>
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <h3 class="font-medium truncate group-hover:text-primary">{{ article.story_title }}</h3>
                  ...
                  <div class="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                    <span class="flex items-center gap-1 group-hover:text-zinc-700 dark:group-hover:text-zinc-300">
                      <Clock class="h-3 w-3" />
                      {{ article.timeAgo }}
                    </span>
                    <span class="flex items-center gap-1 group-hover:text-zinc-700 dark:group-hover:text-zinc-300">
                ...
                <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <Button variant="ghost" size="sm" @click="openArticle(article)"
                          class="hover:bg-zinc-200 dark:hover:bg-zinc-700">
                    <Eye class="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" @click="approveArticle(article)"
                          class="hover:bg-green-100 dark:hover:bg-green-900/30 hover:text-green-600">
                    <Check class="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" @click="rejectArticle(article)"
...
const filteredArticles = computed(() => {
  ...
  return articlesForReview.filter(article => article.status === filterStatus.value);
});
